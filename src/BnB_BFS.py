...
from top_down import flexi_clique
...
def cp_branch_BFS(G, tau):
    ...
    while queue:
        ...
        if S:
            min_deg= min(deg[v] for v in S)

            possilbe_maximum = math.floor((min_deg+1) ** (1 / tau))

            if len(S) >= possilbe_maximum:
                continue
        ...
        while R:
            ...
            if len(S) == 0:
                v = R[0]
                neighbor_decreases = [w for w in G_.neighbors(v) if deg[w] <= math.ceil(theta)]
                if neighbor_decreases:
                    follower = compute_follower(G_, v, theta)
                else: follower = set()

            else:
                neighbor_decreases = [u for u in R if u in G_ and any(deg[w] <= math.ceil(theta) for w in G_.neighbors(u))]
                if neighbor_decreases:
                    v, follower = best_collapser(G_, deg, theta, R)
                else:
                    v = max(R, key=lambda u: deg[u])
                    follower = set()
            ...
            if len(S) >= possilbe_maximum or best_size >= possilbe_maximum:
                # print("skipped")
                continue
            ...
            old_cr_filtered = set(Cr) - set(new_S) - set(new_D)


            new_neighbors = {w for w in node_info[v]['neighbors'] if w not in set(new_S) and w not in new_D}

            new_Cr = list(old_cr_filtered  | new_neighbors)
...
def find_max_flexiclique(G, tau):
    ...
    if not H:
        return cand, best_size, initial_best_size, len(subgraph.edges())
    ...
    if not best_flexiclique:
        return cand, best_size, initial_best_size, len(subgraph.edges())
    return best_flexiclique, best_size, initial_best_size, len(G.subgraph(best_flexiclique))
...
if __name__ == "__main__":
    ...
    start = time.time()
    result, size, initial_best_size, length= find_max_flexiclique(G, tau)
    end = time.time()
    ...
    output_filename = f"{tau}_BnB.dat"
    ...
    with open(output_path, 'w') as output_file:
        ...
        output_file.write(f"{tau}-clique: # of edges: {len(subgraph_edges)}\n")
