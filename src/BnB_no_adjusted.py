...
###################################
        while R:
            if set(S).intersection(prev_D):
                # S에 이미 discarded된 노드가 있다면 상태 무시
                break
            new_D = D.copy()
            ...
            if len(S) == 0:
                ...
                if neighbor_decreases:
                    # print("follower", neighbor_decreases)
                ...
                if neighbor_decreases:
                    ...
                    # node in deg.key() with the largest degree
            ...
            if len(new_S) > best_size and is_flexiclique(new_S, G, tau):
                ...
                if math.floor((best_size+1)**tau-1) > theta:
                    ...
                    prev_D = new_D
                    print("Global discarded updated:", discarded)
...
def find_max_flexiclique(G, tau):
    ...
    global best_flexiclique, best_size, discarded, node_info, initial_order, theta
    best_flexiclique = set()
    # comp, T, cores,k = k_core_seed(G, tau)
    # print("core size",len(T))
...
if __name__ == "__main__":
    ...
    output_filename = f"{tau}_BnB_no_adjusted.dat"
